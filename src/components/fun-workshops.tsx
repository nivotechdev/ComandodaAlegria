import { Paintbrush, Puzzle, Wand } from "lucide-react";
import { Card, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import Link from "next/link";
import { cn } from "@/lib/utils";

const Wave = () => (
    <div className="absolute top-0 left-0 w-full overflow-hidden leading-[0] -translate-y-px">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none" className="relative block w-[calc(100%+1.3px)] h-[80px] fill-accent/10">
            <path d="M321.39,56.44c58-10.79,114.16-30.13,172-41.86,82.39-16.72,168.19-17.73,250.45-.39C823.78,31,906.67,72,985.66,92.83c70.05,18.48,146.53,26.09,214.34,3V120H0V27.35A600.21,600.21,0,0,0,321.39,56.44Z"></path>
        </svg>
    </div>
)

const workshops = [
  {
    icon: <Puzzle className="h-10 w-10 text-secondary" />,
    title: "Recreação Animada",
    description: "Brincadeiras clássicas, gincanas e muita energia para não deixar ninguém parado! Nossa equipe de recreadores transforma qualquer espaço em um parque de diversões.",
  },
  {
    icon: <Paintbrush className="h-10 w-10 text-accent" />,
    title: "Pinturinha Facial",
    description: "Transformamos as crianças em seus heróis e animais favoritos com tintas antialérgicas e muito brilho! Uma recordação colorida que encanta a todos.",
  },
  {
    icon: <Wand className="h-10 w-10 text-vibrant-pink" />,
    title: "Escultura em Balões",
    description: "Arte com balões que ganham formas de espadas, bichinhos e flores. O presente perfeito para cada convidado levar para casa!",
  },
]

export default function FunWorkshops() {
  return (
    <section id="workshops" className="w-full py-20 md:py-32 lg:py-40 bg-accent/10 relative">
      <Wave />
      <div className="container mx-auto px-4 md:px-6 relative">
        <div className="flex flex-col items-center justify-center space-y-4 text-center mb-12">
            <h2 className="font-headline text-4xl font-extrabold tracking-tight sm:text-5xl md:text-6xl" style={{ textShadow: '3px 3px 0px white' }}>
                <span className="text-secondary inline-block -rotate-2">Muito Mais</span>{' '}
                <span className="text-accent inline-block rotate-1">que</span>{' '}
                <span className="text-primary inline-block">Personagens!</span>
            </h2>
            <p className="max-w-[900px] text-muted-foreground md:text-xl/relaxed lg:text-base/relaxed xl:text-xl/relaxed">
              Além dos seus heróis e princesas favoritos, oferecemos oficinas que completam a diversão.
            </p>
        </div>
        <div className="grid gap-8 md:grid-cols-3">
            {/* Card 1 */}
            <Card 
              className="flex flex-col items-center text-center p-8 rounded-3xl shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-2 hover:scale-105 animate-bounce-in border-4 bg-card border-secondary hover:shadow-secondary/30"
              style={{ animationDelay: `0ms` }}
            >
              <CardHeader className="p-0">
                <div className="rounded-full p-4 mb-4 inline-block bg-secondary/10">
                    {workshops[0].icon}
                </div>
                <CardTitle className="text-2xl font-bold font-headline">{workshops[0].title}</CardTitle>
              </CardHeader>
              <CardDescription className="mt-4 text-base">
                {workshops[0].description}
              </CardDescription>
            </Card>
            {/* Card 2 */}
            <Card 
              className="flex flex-col items-center text-center p-8 rounded-3xl shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-2 hover:scale-105 animate-bounce-in border-4 bg-card border-accent hover:shadow-accent/30"
              style={{ animationDelay: `150ms` }}
            >
              <CardHeader className="p-0">
                <div className="rounded-full p-4 mb-4 inline-block bg-accent/10">
                    {workshops[1].icon}
                </div>
                <CardTitle className="text-2xl font-bold font-headline">{workshops[1].title}</CardTitle>
              </CardHeader>
              <CardDescription className="mt-4 text-base">
                {workshops[1].description}
              </CardDescription>
            </Card>
            {/* Card 3 */}
            <Card 
              className="flex flex-col items-center text-center p-8 rounded-3xl shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-2 hover:scale-105 animate-bounce-in border-4 bg-card border-vibrant-pink hover:shadow-vibrant-pink/30"
              style={{ animationDelay: `300ms` }}
            >
              <CardHeader className="p-0">
                <div className="rounded-full p-4 mb-4 inline-block bg-vibrant-pink/10">
                    {workshops[2].icon}
                </div>
                <CardTitle className="text-2xl font-bold font-headline">{workshops[2].title}</CardTitle>
              </CardHeader>
              <CardDescription className="mt-4 text-base">
                {workshops[2].description}
              </CardDescription>
            </Card>
        </div>
        <div className="mt-16 text-center">
            <p className="text-xl font-medium text-foreground/80 mb-4">Quer o pacote completo?</p>
            <Button asChild size="lg" className="bg-primary text-primary-foreground rounded-full px-8 py-7 text-lg font-bold shadow-lg hover:shadow-xl transition-all hover:scale-110 active:scale-105">
                <Link href="#contact">Consultar Combos</Link>
            </Button>
        </div>
      </div>
    </section>
  );
}
